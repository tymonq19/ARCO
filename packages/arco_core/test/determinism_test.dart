  ...
  group('hash', () {
    ...
    test('is a 32-bit value and reacts to every serialized field', () {
      ...
      final mutations = <String, void Function(GameState)>{
        ...
        'ball.x': (g) => g.ball.x += 1e-4,
        'ball.y': (g) => g.ball.y += 1e-4,
        'ball.vx': (g) => g.ball.vx += 1e-4,
        'ball.vy': (g) => g.ball.vy += 1e-4,
        'ball.speed': (g) => g.ball.speed += 1e-4,
        'ball.owner': (g) => g.ball.owner = g.ball.owner == 0 ? 1 : 0,
        'ball.active': (g) => g.ball.active = !g.ball.active,
        'wall': (g) => g.walls.add(
          Wall(id: 77, x1: 0.1, y1: 0.1, x2: 0.3, y2: 0.3, ttl: 600),
        ),
