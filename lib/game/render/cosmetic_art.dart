...
///
/// Two calls per frame, in the painter's own order: the wake goes down first
/// (under the paddles, like the trail always has), then the body.
abstract class BallArt extends CosmeticArt {
  /// The catalogue item this draws.
  BallSkin get skin;

  /// The wake behind the ball, in screen space. Called once per frame before the
  /// body, and only while the ball is live.
  ...
  /// Screen-space wake points, newest first, walking back along the ball's real
  /// path until [maxLength] simulation units of it have been covered.
  ///
  ...
  int collectWake(
    ...
    final n = fx.trailCount;
    if (n < 2) return 0;
    final limit = math.min(maxPoints, n);
...
/// The three colours a prism splits the palette into.
///
...
/// Modernist's paper, and cyan / pink / steel in Glass.
List<Color> prismEdges(GameTheme theme) => <Color>[
  theme.trail,
  theme.heart,
  theme.wall,
];
...
Color emberHot(GameTheme theme) => theme.isDark ? theme.star : theme.accent;

/// The colour an ember's sparks fly in.
Color emberSpark(GameTheme theme) => theme.heart;
